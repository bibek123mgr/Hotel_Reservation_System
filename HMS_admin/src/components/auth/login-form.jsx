...
import { getAllHotel, loginUser } from "../../app/features/auth/authSlice";
import { unwrapResult } from "@reduxjs/toolkit";
import { Mail, Lock, ChevronDown } from "lucide-react";

// Zod schema for form validation
const loginSchema = z.object({
    hotelCode: z.string().min(1, { message: "Please select a hotel" }),
    email: z
        .string()
...
const LoginForm = ({ onSuccess, onForgotPassword }) => {
    ...
    useEffect(() => {
        const fetchHotels = async () => {
            try {
                const resultAction = await dispatch(getAllHotel());
                if (getAllHotel.fulfilled.match(resultAction)) {
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-2">
                ...
                <div className="relative mt-4">
                    <select
                        ...
                        id="hotel-select"
                        className="w-full pl-3 pr-10 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 focus:outline-none transition-all appearance-none h-12"
                        disabled={isLoading || isLoadingHotels}
                    >
                        <option value="" className="text-gray-500">Select a hotel</option>
                        {isLoadingHotels ? (
                            <option value="" disabled className="text-gray-400">Loading hotels...</option>
                        ) : (
                            hotels.map(hotel => (
                                <option key={hotel.id} value={hotel.hotelCode} className="text-gray-800">
                                    {hotel.hotelName}
                                </option>
                            ))
                        )}
